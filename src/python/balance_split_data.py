...
def group_by_case(df_train, df_test, df_test1, df_excluded):
    # Group df_train by 'case_num', 'article', 'file', and join the 'text_clean' column
    
    df_train_grouped = df_train.groupby(['year', 'case_num', 'article', 'label'])['text_clean'].agg(' '.join).reset_index() #'file',
    ...
    # Group df_test by 'case_num', 'article', 'file', and join the 'text_clean' column
    df_test_grouped = df_test.groupby(['year', 'article', 'case_num', 'label'])['text_clean'].agg(' '.join).reset_index()
    ...
    df_test1_grouped = df_test1.groupby(['year', 'article', 'case_num', 'label'])['text_clean'].agg(' '.join).reset_index()
    ...
    # Group df_excluded by 'case_num', 'article', 'file', and join the 'text_clean' column
    df_excluded_grouped = df_excluded.groupby(['year', 'article', 'case_num', 'label'])['text_clean'].agg(' '.join).reset_index()
