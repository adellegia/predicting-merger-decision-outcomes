...
def fit_best_model_train(X_train, y_train, model, best_parameters, cv): #cv =3
    ...
    y_predict = cross_val_predict(pipeline_test, X_train, y_train, cv=cv)
    evaluate(y_train, y_predict)
    return(pipeline_test, y_predict)
...
def get_feature_importance_cv(pipeline_name):
    ...
    df_features = pd.DataFrame({'Feature Name': feature_names, 'Importance': feature_importances})
...
def train_model_cross_val(Xtrain, Ytrain, vec, model, cv): #Linear SVC model cross-validation
    ...
    Ypredict_train = cross_val_predict(pipeline, Xtrain, Ytrain, cv=cv) #10-fold cross-validation
    evaluate(Ytrain, Ypredict_train)
    return(pipeline, Ypredict_train)
